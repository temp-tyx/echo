...
public:
    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint64_t batch_i = 0; batch_i < B_; batch_i++) {
            ...
            int32_t seq0 = seq1;
            seq1 += seqLen;
            int32_t ssmSeq0 = cuSeqlensGm_.GetValue(0) + static_cast<int32_t>(batch_i * colCount_);
            uint32_t copyFlag = 0;
...
private:
    ...
    __aicore__ inline void ProcessHead(int32_t seq0, int32_t seq1, uint64_t head_i, uint64_t stateOffset, int32_t ssmSeq0)
    {
        ...
        for (uint64_t v_i = 0; v_i < realV_; v_i += vStep_) {
            ...
            for (uint64_t seq_i = seq0; seq_i < seq1; seq_i++) {
                ...
                uint64_t ssmSeq_i = ssmSeq0 + (seq_i - seq0);
                uint64_t curStateOutOffset =
                    ((ssmStateIndicesGm_.GetValue(ssmSeq_i) * NV_ + head_i) * realV_ + v_i) * realK_;
