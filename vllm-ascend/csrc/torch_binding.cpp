...
#include "attention/fused_gather_logsumexp/fused_gather_logsumexp_torch_adpt.h"
...
TORCH_LIBRARY_EXPAND(CONCAT(_C, _ascend), ops)
{
    ...
    // Fused gather + logsumexp.
    ops.def(
        "npu_fused_gather_logsumexp(Tensor logits, "
        "                          Tensor draft_tokens) "
        "                          -> (Tensor output)");
    ops.impl("npu_fused_gather_logsumexp", torch::kPrivateUse1, &vllm_ascend::npu_fused_gather_logsumexp);
}
#endif
